...
class Trainer_adv:
    ...
    def _combine_networks(self):
        ...
        if self.feature_extractor:
            ...
            for i in range(len(sr)-n):
                ...
                loss_weights.extend(
                   [self.loss_weights['feature_extractor'] / len(sr_feats) *(1+i/3.0)] * len(sr_feats)
                 )
    ...
    def train(self, epochs, steps_per_epoch, batch_size, monitored_metrics):
        ...
        for index in range(self.mag_num):
            if index==0:
                validation_set[index],top_left_fronts,transform_before = self.valid_dh[index].get_validation_set(batch_size)
            else:
                validation_set[index], _ ,_ = self.valid_dh[index].get_validation_set(batch_size,top_left_fronts,transform_before)

        y_validation = [validation_set[index]['hr'] for index in range(self.mag_num)]
        ...
        if self.discriminator:
            ...
            validation_valid = np.ones([len(validation_set[self.mag_num-1]['hr'])] + discr_out_shape)
            y_validation.append(validation_valid)

        if self.feature_extractor:
            for index in range(self.mag_num):
                validation_feats = self.feature_extractor.model.predict(validation_set[index]['hr'])
        ...
        for epoch in range(starting_epoch, epochs):
            ...
            for step in tqdm(range(steps_per_epoch)):
                ...
                for index in range(self.mag_num):
                    if index==0:
                        batch[index],top_left_front,idx,transform_before = self.train_dh[index].get_batch(batch_size, flatness=flatness)
                    else:
                        batch[index],temp,idx,_=self.train_dh[index].get_batch(batch_size,idx,
                                                                      flatness=flatness,top_left_front=top_left_front
                ...
                if self.discriminator:
                    ...
                    d_loss_real = self.discriminator.model.train_on_batch(batch[self.mag_num-1]['hr'], valid)
                    d_loss_fake = self.discriminator.model.train_on_batch(sr[-1], fake)
                ...
                if self.feature_extractor:
                    for index in range(self.mag_num):
                       hr_feats = self.feature_extractor.model.predict(batch[index]['hr'])
                       y_train.extend([*hr_feats])

                model_losses = self.model.train_on_batch(batch[0]['lr'], y_train)
            ...
            validation_losses = self.model.evaluate(
                validation_set[0]['lr'], y_validation, batch_size=batch_size
            )
