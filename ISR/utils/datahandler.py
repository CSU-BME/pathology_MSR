...
class DataHandler:
    ...
    def __init__(self, lr_dir, hr_dir, patch_size, scale, n_validation_samples=None,index=None):
        ...
        self.index=None
        self._make_img_list(index)
        self._check_dataset()
    
    def _make_img_list(self,index=None):
        """ Creates a dictionary of lists of the acceptable images contained in lr_dir and hr_dir. """
        #index get some images by index
    ...
    def _crop_imgs(self, imgs, batch_size, flatness,top_lef_front=None):
        """
        ...
        top left corners.
        top_left_front: front layer for multiple models connected
        """
    ...
    def get_batch(self, batch_size, idx=None, flatness=0.0,top_left_front=None,transform_before=None):
        """
        ...
        batch,top_left = self._crop_imgs(img, batch_size, flatness,top_left_front)

        if transform_before is None:
            transforms = np.random.randint(0, 3, (batch_size, 2))
        else:
            transforms=transform_before

        batch['lr'] = self._transform_batch(batch['lr'], transforms)
        batch['hr'] = self._transform_batch(batch['hr'], transforms)
        
        return batch,top_left,idx,transforms
    
    def get_validation_batches(self, batch_size,top_left_fronts=None,transform_before=None):
        ...
        if self.n_validation_samples:
            ...
            for idx in range(self.n_validation_samples):
                if top_left_fronts is None:
                   batch,top_left,idx,transform=self.get_batch(batch_size, idx,flatness=0.0)
                else:
                   batch, top_left, idx,transform = self.get_batch(batch_size, idx,
                                                          flatness=0.0,top_left_front=top_left_fronts[idx],
                                                                    transform_before=transform_before[idx])
                batches.append(batch)
                top_lefts.append(top_left)
                transforms.append(transform)

            return batches,top_lefts,transforms
        else:
            self.logger.error(
    ...
    def get_validation_set(self, batch_size,top_left_fronts=None,transform_before=None):
        """
        Returns a batch for each image in the validation set.
        ...
        if self.n_validation_samples:
            batches,top_lefts,transforms = self.get_validation_batches(batch_size,top_left_fronts,transform_before)
            valid_set = {'lr': [], 'hr': []}
            ...
            return valid_set,top_lefts,transforms
        else:
            self.logger.error(
                'No validation set size specified. (not operating in a validation set?)'
            )
