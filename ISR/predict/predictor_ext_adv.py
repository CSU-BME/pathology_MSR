...
class Predictor_ext_adv:
    ...
    def __init__(self, input_dir, output_dir='./data/output', verbose=True):
        ...
        self.extensions = ('.jpeg', '.jpg', '.png','.BMP')  # file extensions that are admitted
    ...
    def _load_weights(self):
        ...
        session_config_path = self.weights_path_posix.parent / 'session_config.yml'
        if session_config_path.exists():
            ...
            conf = {}
        conf.update({'pre-trained-weights': self.weights_path_posix.name})
        return conf
    ...
    def get_predictions(self, model, weights_path,patch_size=None):
        """ Runs the prediction. """
        ...
        self.weights_path_posix=Path(weights_path)
        ...
        for img_path in self.img_ls:
            ...
            for i in range(len(sr_img)):
              imageio.imwrite(output_path / (str(i)+'_' + img_path.name), sr_img[i])

    def _forward_pass(self, file_path):
        lr_img = imageio.imread(file_path)
        if lr_img.shape[2] == 3:
            sr_img = self.model.predict(lr_img,by_patch_of_size=self.patch_size-4)
            return sr_img
        else:
